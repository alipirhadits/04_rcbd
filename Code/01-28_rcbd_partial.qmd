...
rcbd_means_all <- emmeans(rcbd_mod, ~nrate_kgha:krate_kgha)
                  # emmeans(rcbd_mod, ~nrate_kgha | krate_kgha)
...
ggplot() + 
  ...
  labs(x = " N rate + K rate(kgha)", #برای اضافه کردن لیبل دلخواه برای محور اکس و وای اضافه شد
       y = "Yield (kgha)") +
  scale_fill_viridis_d() +
  scale_color_viridis_d() +
  theme(legend.position = "none") # برای حذف کردن لنجد استفاده شد
...
ggplot() + 
  ...
  theme(legend.position = "none") # برای حذف کردن لنجد استفاده شد
ggsave("../Output/rcbd_means_Krates_inside_N.png",
       height = 4,
       width = 6)
```
```
