...
# Model fitting
fixed_effect_model <- lm(yield_kgha ~ nrate_kgha * krate_kgha + rep, data = rcbd_dfw)
random_effect_model <- lmer(yield_kgha ~ nrate_kgha * krate_kgha + (1|rep), data = rcbd_dfw)
...
# f) ANOVA table  
```{r ANOVA}
anova_fixed <- Anova(fixed_effect_model, type=3)
anova_random <- Anova(random_effect_model, type=3)
```
